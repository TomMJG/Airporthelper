...
- (void)createImageView
{
    ...
    _showrouteview = [[ShowRouteView alloc] initWithFrame:CGRectZero];
    _showrouteview.backgroundColor =  [UIColor clearColor];
    [self addSubview:self.showrouteview];
    
}


- (void)addshowRouteView {
    [_showrouteview removeFromSuperview];
    _showrouteview = [[ShowRouteView alloc] initWithFrame:CGRectZero];
    _showrouteview.backgroundColor =  [UIColor clearColor];
    self.showrouteview.frame = self.imageView.frame;
    [self addSubview:self.showrouteview];
    //NSLog(@"create showrouteview");
}

//- (void)removeshowroutevie

- (void)drawRect:(CGRect)rect{
    //NSLog(@"call drawrect1");
...
//    NSLog(@"mapsize width");
//    NSLog(@"%f",map.size.width);
//    NSLog(@"mapsize height");
//    NSLog(@"%f",map.size.height);
    self.showrouteview.frame = self.imageView.frame;
...
- (void)changemap:(UIImage *)map
{
    self.imageView.image = map;
}

- (void)addAnnotation:(NAPinAnnotation *)annotation animated:(BOOL)animate
{
...
//修改
- (void)addCurrentAnnotation:(NAPinAnnotation *)annotation animated:(BOOL)animate
{
    annotation.color = NAPinColorGreen;
    [annotation addToMapView:self animated:animate];
}
...
- (void)removeAnnotation:(NAPinAnnotation *)annotation
{
...
//修改
- (void)removeCurrentAnnotation:(NAAnnotation *)annotation
{
    [annotation removeFromMapView];
}
